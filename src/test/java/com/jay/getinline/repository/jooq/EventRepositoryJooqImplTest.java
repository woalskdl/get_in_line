...
class EventRepositoryJooqImplTest {
    ...
    @DisplayName("이벤트 데이터를 검색 파라미터와 함께 조회하면, 조건에 맞는 데이터를 페이징 처리하여 리턴한다.")
    @Test
    ...
    @DisplayName("이벤트 데이터 검색 파라미터에 따른 조회 결과가 없으면, 빈 데이터를 페이징 정보와 함께 리턴한다.")
    @Test
    void givenSearchParams_whenFindingNotExistEventViewPage_thenReturnsEmptyEventViewResponsePage() {
    ...
    @DisplayName("이벤트 데이터를 검색 파라미터 없이 페이징 값만 주고 조회하면, 전체 데이터를 페이징 처리하여 리턴한다.")
    @Test
    void givenPagingInfoOnly_whenFindingNotExistEventViewPage_thenReturnsEventViewResponsePage() {
        ...
        Page<EventViewResponse> eventPage = sut.findEventViewPageBySearchParams(
                ...
                null,
                null,
                PageRequest.of(0, 5, Sort.by(Sort.Order.asc("placeName"), Sort.Order.desc("eventStartDateTime")))
        );
